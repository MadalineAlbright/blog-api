...
public class BlogMockedData {
	 ...
	 //list of blog posts
    private List<BlogItemModelClass> blogs;
    ...
    public BlogMockedData(){
        blogs = new ArrayList<BlogItemModelClass>();
        blogs.add(new BlogItemModelClass(1, "Go up, up and away with your Google Assistant",
        ...
        blogs.add(new BlogItemModelClass(2, "Get local help with your Google Assistant",
        ...
        blogs.add(new BlogItemModelClass(3, "The new maker toolkit: IoT, AI and Google Cloud Platform",
        ...
        blogs.add(new BlogItemModelClass(4, "Learn more about the world around you with Google Lens and the Assistant",
        ...
        blogs.add(new BlogItemModelClass(5, "7 ways the Assistant can help you get ready for Turkey Day",
    ...
    // return all blogs
    public List<BlogItemModelClass> fetchBlogs() {
        return blogs;
    }

    // return blog by id
    public BlogItemModelClass getBlogById(int id) {
        for(BlogItemModelClass b: blogs) {
            if(b.getId() == id) {
                return b;
            }
        }
    ...
    // search blog by text.???
    public List<BlogItemModelClass> searchBlogs(String searchTerm) {
        List<BlogItemModelClass> searchedBlogs = new ArrayList<BlogItemModelClass>();
        for(BlogItemModelClass b: blogs) {
    ...
    // create blog
    public BlogItemModelClass createBlog(int id, String title, String content) {
    	BlogItemModelClass newBlog = new BlogItemModelClass(id, title, content);
        blogs.add(newBlog);
        return newBlog;
    }

    // update blog
    public BlogItemModelClass updateBlog(int id, String title, String content) {
        for(BlogItemModelClass b: blogs) {
            if(b.getId() == id) {
    ...
    /* A class to simulate the database functionality.The class will contain a list of hard-coded blog posts.
    The class must be a singleton so that changes can be persisted across different HTTP requests.If a class is a singleton, you can only have one instance of that class throughout the application lifecycle.
